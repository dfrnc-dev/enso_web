if (document.querySelector('.wrapper-start')) {
	...
	const anim = () => {
			...
			for (let i in pMoveArr) {
				...
				tlMainCap
					.to(
						gMove,
						{
							duration: tempParam.time,
							motionPath: {
								path: tempParam.path,
								align: tempParam.path,
								autoRotate: 180,
								alignOrigin: [0.5, 0.5],
							},
							ease: 'none',
					...
					.to(
						gMove + ' .morphF',
						{ duration: tempParam.time, morphSVG: tempParam.morphClass, ease: 'none' },
						'<'
					);
			}
			...
			for (let i in gSplashArr) {
				...
				tlSpalsh.add(
					gsap
						...
						.to(gSplashArr[i], {
							duration: timeAll * splashProgress,
							motionPath: {
								path: pSplash,
								align: pSplash,
								start: 0,
						...
						.to(
							gSplashArr[i],
							{
								duration: timeAll * 0.3,
								scale: 0,
								motionPath: {
									path: pSplash,
									align: pSplash,
									start: splashProgress,
									end: splashProgress + 0.1,
									autoRotate: 180,
									alignOrigin: [-0.2, 0.5],
								},
								ease: 'power1.out',
		...
		gsap.set(
			'#timelineSVG #mark1, #timelineSVG #mark2, #timelineSVG #mark3, #timelineSVG #mark4, #timelineSVG #mark5, #timelineSVG #mark6, #timelineSVG #mark7, #timelineSVG #mark8, #timelineSVG #mark9, #timelineSVG #mark10',
			{ transformOrigin: '50% 100%', scale: 0 }
		);
		gsap.set(
		...
		const wayAn = gsap
			...
			.to(
				'#timelineSVG #way, #timelineSVG #marks',
				{ duration: 0.5, x: -100, ease: 'sine.inOut' },
				'<'
			)
			.to(
				'#timelineSVG #way, #timelineSVG #marks',
				{ duration: 2, x: 200, ease: 'sine.inOut' },
				'>'
			)
			.to(
				'#timelineSVG #way, #timelineSVG #marks',
				{ duration: 1.5, x: 0, ease: 'sine.inOut' },
				'>'
			)
			.to(
				'#timelineSVG #way, #timelineSVG #marks',
				{ duration: 2, x: 300, ease: 'sine.inOut' },
				'>'
			)
		...
		window.addEventListener('load', () => {
			...
			setTimeout(() => {
				...
				document.getElementById('what_new-link').addEventListener('click', () => {
					gsap.to(window, { duration: 0.5, ease: 'none', scrollTo: 0 });
				})
				...
				window.addEventListener('scroll', () => {
					...
					if (window.scrollY > ((10000 * tl.labels.Label1) / tl.duration()) && window.scrollY < 10000) {
						...
						document.querySelector(`#timeline-link`).style.background = 'transparent'
					}
				})
...
function navLink(name) {
	document.getElementById(`${name}-link`).addEventListener('click', () => {
